...
def die_overlaps_existing(candidate_rect, dice):
    ...
    return any(padded_rect.colliderect(die["rect"]) for die in dice)
...
def throw_die(die, power, floor_rect, dice=None):
    ...
    direction = pygame.Vector2(1 if die["rect"].centerx <= floor_rect.centerx else -1, random.uniform(-0.18, 0.18))

    targets = [other_die for other_die in dice or [] if other_die is not die]
    if targets:
        die_center = pygame.Vector2(die["rect"].center)
        target = min(targets, key=lambda other_die: die_center.distance_to(other_die["rect"].center))
...
def clamp_die_to_floor(die, floor_rect):
    die["x"] = max(floor_rect.left, min(die["x"], floor_rect.right - DIE_SIZE))
    die["y"] = max(floor_rect.top, min(die["y"], floor_rect.bottom - DIE_SIZE))
...
def update_die_motion(die, dt_ms, floor_rect):
    ...
    if die["x"] < floor_rect.left:
        die["x"] = floor_rect.left
        die["vx"] = abs(die["vx"]) * WALL_BOUNCE
    elif die["x"] > floor_rect.right - DIE_SIZE:
        die["x"] = floor_rect.right - DIE_SIZE
        die["vx"] = -abs(die["vx"]) * WALL_BOUNCE

    if die["y"] < floor_rect.top:
        die["y"] = floor_rect.top
        die["vy"] = abs(die["vy"]) * WALL_BOUNCE
    elif die["y"] > floor_rect.bottom - DIE_SIZE:
        die["y"] = floor_rect.bottom - DIE_SIZE
        die["vy"] = -abs(die["vy"]) * WALL_BOUNCE
...
def resolve_die_collision(first_die, second_die):
    first_center = pygame.Vector2(first_die["x"] + DIE_SIZE / 2, first_die["y"] + DIE_SIZE / 2)
    second_center = pygame.Vector2(second_die["x"] + DIE_SIZE / 2, second_die["y"] + DIE_SIZE / 2)
    delta = second_center - first_center
    distance = delta.length()
    min_distance = DIE_SIZE
